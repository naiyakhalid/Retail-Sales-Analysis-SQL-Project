...
-- Finding NULL Values
SELECT * FROM Retail_Sales_Analysis WHERE transaction_id IS NULL;  -- NO NULL 
...
SELECT * FROM Retail_Sales_Analysis WHERE  quantity IS NULL; -- Have NULL 
...
WITH mean_vals AS (
    SELECT 
        ...
        CAST(AVG(quantity) AS INT) AS mean_quantiy,
...
UPDATE Retail_Sales_Analysis
SET quantity = (SELECT CAST(AVG(quantiy) AS INT) FROM Retail_Sales_Analysis WHERE quantiy IS NOT NULL)
...
SELECT COUNT(transaction_id) AS Total_no_of_transactions FROM Retail_Sales_Analysis  --- Total_no_of_transactions = 2000

-- How much qunatity/items we have sold in total 
SELECT SUM(quantity) AS Total_quantity_Sold FROM Retail_Sales_Analysis -- Total_items_sold = 5024
...
-- Q.7 Write a SQL query to calculate the average sale for each month. Find out best selling month in each year
SELECT YEAR(sale_date) AS Each_Year, MONTH(sale_date) AS Each_Month, AVG(total_sale) AS Avg_sale_each_month FROM Retail_Sales_Analysis 
GROUP BY MONTH(sale_date),YEAR(sale_date) ORDER BY 1 , 3 DESC 

-- Q.7 Write a SQL query to calculate the sale for each year. Find out best selling year
SELECT YEAR(sale_date) AS sale_Year, SUM(total_sale) AS total_sales_each_year FROM Retail_Sales_Analysis 
GROUP BY YEAR(sale_date) ORDER BY SUM(total_sale) DESC 
